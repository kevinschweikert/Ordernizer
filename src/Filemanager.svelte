    ...
    const createConfigs = (e) => {
        if (selectedPaths.length > 0) {
            selectedPaths.forEach(path => {
                const files = jetpack.dir($sessionPath).dir(path).list().filter(file => file != $cfgFileName)
                jetpack.dir($sessionPath).dir(path).write($cfgFileName, createConfig(path, files), {atomic: true})
            })
        }
        toggleModal()
    }
...
{#if modalActive}
     <ConfigModal on:toggle={toggleModal} on:create={createConfigs} paths={pathsToCreate} bind:selected={selectedPaths}/>
{/if}
